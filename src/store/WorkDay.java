        ...
        if(listOfOrderedDrinks.isEmpty()){
            ...
            for(AcquisitionUnit unit : listOfOrderedDrinks){
                ...
                if(counter > 2){
                    ...
                    System.out.print(" 7%  *** ");
                    System.out.printf("%-2s", Float.toString(unit.getPurchase_priceWithExtraCharge(0.07f)));
                }else{
                    if(CURRENT_TIME.plusMinutes(1).isAfter(TIME_DISCOUNT_START) && CURRENT_TIME.plusMinutes(1).isBefore(TIME_DISCOUNT_END)){
                        ...
                        System.out.print(" 8%  *** ");
                        System.out.printf("%-2s",Float.toString(unit.getPurchase_priceWithExtraCharge(0.08f)));
                    }else{ if(isWeekend){
                                cleanProfitOfTheDay += unit.getPurchase_price()*0.15;
                                System.out.print(" 15% *** ");
                                System.out.printf("%-2s",Float.toString(unit.getPurchase_priceWithExtraCharge(0.15f)));
                            }else{
                                    cleanProfitOfTheDay += unit.getPurchase_price()*0.1;
                                    System.out.print(" 10% *** ");
                                    System.out.printf("%-2s",Float.toString(unit.getPurchase_priceWithExtraCharge(0.1f)));
                                }
                    }
                }
